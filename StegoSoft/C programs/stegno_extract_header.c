...
/*
 * open a file for writing the Header information
 */
	output_message_file = fopen( "./HeaderInfo.h", "w" );
	fprintf( output_message_file, "%d\n", carrier_version);
	fprintf( output_message_file, "%d\n", header_len);
	fprintf( output_message_file, "%d\n", bit_gap);
	fprintf( output_message_file, "%d\n", encrypt_all);
	fprintf( output_message_file, "%d\n", no_of_message_files);
	if ( encrypt_all != 0 ){
		fprintf( output_message_file, "%d\n", password_size);
		fprintf( output_message_file, "%s\n", password_carrier);
	}
	for( i=0; i<no_of_message_files; i++){
		fprintf( output_message_file, "%d\n", message_details[i].file_size);
		fprintf( output_message_file, "%d\n", message_details[i].filename_size);
		fprintf( output_message_file, "%d\n", message_details[i].encryption);
		fprintf( output_message_file, "%s\n", message_details[i].filename);
	}
